...
let talents: any[] = [
  {
    ...
    userId: "user_alex",
    user: users[0],
    title: "Python, Django REST Framework & Production Backend Architecture",
...
app.get("/api/talents", (req, res) => {
  const { q, category, format } = req.query;
  let results = [...talents];
...
app.post("/api/swaps", (req, res) => {
  try {
    ...
    const newSwap = {
      ...
      requesterOfferTitle: requesterOfferTitle || "Skill Exchange Offer",
      requesterOfferDescription: requesterOfferDescription || "Mutual peer learning session",
      status: "escrow_deposit_required",
      sessionDateProposal: sessionDateProposal || "Upcoming weekend",
      sessionNotes: "Awaiting NOWPayments escrow deposit to lock mutual commitment.",
      meetingLink: `https://meet.google.com/swap-${Math.random().toString(36).substring(7)}`,
      escrowDepositUSD: Number(escrowDepositUSD) || listing.escrowDepositUSD || 15.00,
    ...
    res.status(500).json({ error: err.message || "Failed to propose swap" });
  }
});
...
app.post("/api/swaps/:id/confirm-delivery", (req, res) => {
  ...
  // Check if both peers confirmed delivery
  ...
  res.json({
    success: true,
    swap,
    bothCompleted: swap.requesterDelivered && swap.recipientDelivered
  });
});
