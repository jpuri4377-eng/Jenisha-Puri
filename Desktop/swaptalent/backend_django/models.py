...
class UserProfile(models.Model):
    ...
    crypto_wallet_address = models.CharField(max_length=100, blank=True, help_text="User's payout address for refunded escrow")
...
class TalentListing(models.Model):
    ...
    title = models.CharField(max_length=200, help_text="What you want to teach (e.g., Python & Django Backend Architecture)")
    category = models.CharField(max_length=50, choices=CATEGORY_CHOICES, default='Programming & Tech')
    description = models.TextField(help_text="Detailed description of what you will teach")
    topics_covered = models.JSONField(default=list, help_text="List of topics or curriculum syllabus")
    teach_skills = models.JSONField(default=list, help_text="Skills taught e.g., ['Django', 'REST APIs', 'PostgreSQL']")
    wanted_skills = models.JSONField(default=list, help_text="Skills wanted in exchange e.g., ['React', 'TailwindCSS']")
    proficiency_level = models.CharField(max_length=20, choices=LEVEL_CHOICES, default='Advanced')
    session_format = models.CharField(max_length=40, choices=FORMAT_CHOICES, default='1-on-1 Live Video')
    session_duration_mins = models.PositiveIntegerField(default=60, help_text="Length of each session in minutes")
    ...
    student_prerequisites = models.TextField(blank=True, default='Basic curiosity and laptop with browser')
...
class SwapRequest(models.Model):
    ...
    requester = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='sent_swap_requests')
    recipient = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='received_swap_requests')
    talent_listing = models.ForeignKey(TalentListing, on_delete=models.CASCADE, related_name='swaps')
    requester_offer_title = models.CharField(max_length=200, help_text="What skill the requester will teach in return")
    requester_offer_description = models.TextField(blank=True)
    status = models.CharField(max_length=30, choices=STATUS_CHOICES, default='escrow_deposit_required')
    session_date_proposal = models.CharField(max_length=100, blank=True)
    meeting_link = models.URLField(blank=True, default='https://meet.google.com/swap-talent-room')
    ...
    requester_delivered = models.BooleanField(default=False, help_text="Requester confirmed their session was taught")
    recipient_delivered = models.BooleanField(default=False, help_text="Recipient confirmed their session was taught")
    escrow_released = models.BooleanField(default=False, help_text="NOWPayments escrow refunded upon dual confirmation")
    ...
    def check_and_complete_swap(self):
        ...
        if self.requester_delivered and self.recipient_delivered and not self.escrow_released:
