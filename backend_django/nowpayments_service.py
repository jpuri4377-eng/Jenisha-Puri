...
class NowPaymentsService:
    ...
    def check_api_status(self):
        ...
        try:
            res = requests.get(
                f"{self.base_url}/status", headers=self._get_headers(), timeout=5
            )
            return res.json()
    ...
    def get_available_currencies(self):
        ...
        try:
            res = requests.get(
                f"{self.base_url}/currencies", headers=self._get_headers(), timeout=5
            )
            return res.json()
        except Exception as e:
            return {
                "currencies": ["usdttrc20", "btc", "eth", "sol", "trx", "doge", "ltc"]
            }
